...
#' @param add (character) additional groups to add (so far only "allMeans" available) in addition to the means used in the pairwise comparison
...
#' @export
extractTestingResults <- function(stat, compNo=1, statTy="BH",thrsh=0.05, FCthrs=1.5, annotCol=c("Accession","EntryName","GeneName"), nSign=6,add="allMeans",filename=NULL,fileTy="csvUS",silent=FALSE,callFrom=NULL) {
  ##
  ...
  avCol <- .sampNoDeMArrayLM2(stat, compNo, lstP=useFdrTy)          # ultimately switch to function in wrMisc
