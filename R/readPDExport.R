...
#' @param specPref (character) define characteristic text for recognizing (main) groups of species (1st for comtaminants - will be marked as 'conta', 2nd for main species- marked as 'mainSpe', and optional following ones for supplemental tags/species - maked as 'species2','species3',...)
#' @param separateAnnot (logical) if \code{TRUE} output will be organized as list with \code{$annot}, \code{$abund} for initial/raw abundance values and \code{$quant} with final normalized quantitations
...
#' 
#' @export
readPDExport <- function(fileName, path=NULL, normalizeMeth="median", sampleNames=NULL, read0asNA=TRUE,
  annotCol=NULL, quantCol="^Abundances", contamCol="Contaminant", refLi=NULL, separateAnnot=TRUE, plotGraph=TRUE, tit="Proteome Discoverer", graphTit=NULL, wex=1.6,
  ...
  if("try-error" %in% class(tmp)) stop("Unable to read inout file !")   
  ...
  ## option for future: also extract peptide counts per sample (as done with MQ)
  ## lateron rename columns called as "Description" to annotCol[2]  ??
  ...
  if(sum(!is.na(annotColN2)) > sum(!is.na(annotColNo))) { annotColNo <- annotColN2
    annotCol <- annotCol2
    if(!silent) message(fxNa," correcting 'annotCol' to export of R-friendly colnames")}  
  ...
  annot <- cbind(Accession=annot[,"Accession"], ProteinName=NA, Species=NA, Contam=NA, SpecType=NA, annot[,-1])                        # may be better to name column 'specie
  for(i in 1:length(specPref)) {         # locate specPref
    naSp <- if(i==1) "conta" else {if(i==2) "mainSpe" else paste0("species",i-1)}
    chSp <- grep(specPref[i], annot[,annotCol[2]])
    if(length(chSp) >0) {
  ...
  if("Contaminant" %in% colnames(annot)) annot[,"Contam"] <- toupper(gsub(" ","",annot[,contamCol]))
  ...
  if(!silent) { chSp <- is.na(annot[,"Species"])
    ...
    if(!all(chSp)) { tab <- table(annot[,"Species"])
      ...
      if(!silent) message(fxNa,"   by species : ",apply(tab,2,paste)) }}             # all lines assigned   
  ...
  if(length(quantCol) >1) { 
    ...
    if(length(quantCol) <1) { quantCol <- grep(tolower(quantCol), tolower(colnames(tmp)))
      if(!silent) message(fxNa," Could not find any quantification columns, trying all as lower caps : now ",length(quantCol)) }    
    ...
    colnames(abund) <- sub("^Abundances_{0,1}|^abundances_{0,1}","",colnames(abund))
    }                                        # abundance val
  ...
  if(any(chAc)) {
    ...
    if(TRUE) {                # ready to correct (if possible) duplicated 'Accession' entries
      ...
      ## need to remove duplic&ted lines which are not marked as Contaminant="True" 
      ...
      if(length(rmAnnot) >0) annot <- annot[-rmAnnot,]
  ...
  ## final output
  if(separateAnnot) list(raw=abund, quant=quant, annot=annot, notes=notes) else data.frame(quant,annot)  
}
