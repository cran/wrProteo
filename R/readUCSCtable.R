...
readUCSCtable <- function(fiName, exportFileNa=NULL, gtf=NA, simplifyCols=c("gene_id","chr","start","end","strand","frame"), silent=FALSE, callFrom=NULL) {
  ...
  chGz <-  R.utils::isGzipped(fiName)   #length(grep("\\.gz$",fiName)) >0
  ...
  if(length(simplifyCols) >5) { 
    ...
    if(length(exportFileNa) >0) {   # also need to remove version-tags (since UniProt won't recognize Ensemble gene IDs with ersion tags)
