...
#' head(datT6b$data)
#' @export
matrixNAneighbourImpute <- function(dat,gr,retnNA=TRUE,avSdH=c(0.18,0.5),avSdL=c(0.1,0.5),plotHist=TRUE,xLab=NULL,tit=NULL,seedNo=2018,silent=FALSE,callFrom=NULL){
  ...
  for(i in c(1:length(levels(gr)))) {
    ...
    useLi <- which(nNAmat[,i] >0 & nNAmat[,i] < maxCol-1)           # 1 or 2 NA
    useL2 <- which(nNAmat[,i] >1 & nNAmat[,i] < maxCol-1)           # just 2 NAs
  ...
  if(!silent) message(fxNa," any NA-neighbours ",length(NAneig)," :  use >1 'NA' : ",length(NAneig2) >300," , ie 'NAneig2' of ",length(NAneig2))
  ...
  randParam <- c(mean=randParam[1],sd=randParam[2],seed=seedNo)
  ...
  if(plotHist){
    ...
    ## display 3%tile of init data
    graphics::abline(v=stats::quantile(dat,0.03,na.rm=TRUE),lty=2,col="red")
    ...
    graphics::mtext(paste(" n.woNA=",sum(!isNA),"\n  n.NA =",sum(isNA)),side=3,line=-0.7,cex=0.55,adj=0,col=grDevices::grey(0.3))
    graphics::legend("topright",c("final","initial","infered"),col=colPanel,text.col=colPanel,cex=0.8,seg.len=0.3,lwd=4)}
...
.imputeNA <- function(dat,gr,impParam,exclNeg=TRUE,inclLowValMod=TRUE) {
  ...
  isNa <- is.na(dat)
  set.seed(as.integer(impParam[3])); lowValMod <- stats::rnorm(round(1.5*sum(isNa)),mean=impParam[1],sd=impParam[2])
