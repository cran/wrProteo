...
readProlineFile <- function(fileName, path=NULL, normalizeMeth=NULL, logConvert=TRUE, sampleNames=NULL, quantCol="^abundance_", 
  ...
  if(length(grep("\\.xlsx$", fileName)) >0) {
    ...
    sheets <- if(debug) readxl::excel_sheets(paFi) else suppressMessages(readxl::excel_sheets(paFi))
    ...
    if(debug) message(fxNa,"Ready to use sheet ",prSh)
    ...
    if(debug) message(fxNa,"Reading flat file, best as type no ",bestT," , ie as ",c("txt","US-csv","Euro-csv","tsv (US)","tsv (Euro)")[bestT])
    ...
    if(file.exists(paFi2)) {
      ...
      if(debug) message(fxNa," file '",paFi2,"' not found for quantifaction specific information")}
    ## adopt annotCol
