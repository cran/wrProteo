...
of providing a uniform format for documenting experimental meta-data (ie the sdrf-format).
A few experiments have already been integrated there and can be directly accessed from [wrProteo](https://CRAN.R-project.org/package=wrProteo).
You don't necessarily need to copmy/pase the full path, you only need the main [PRIDE](https://www.ebi.ac.uk/pride/) accession number, `readSdrfAnnot` can compose the full url for you.

For this specific example the meta-data can be found as [https://github.com/bigbio/proteomics-metadata-standard/blob/master/annotated-projects/PXD001819/PXD001819.sdrf.tsv](https://github.com/bigbio/proteomics-metadata-standard/blob/master/annotated-projects/PXD001819/PXD001819.sdrf.tsv).
...
In most 'Omics' acitivities getting additional annotation may get sometimes a bit tricky.
...
a) Strategies using online-based ressources for getting the most up-to-date information/annotation (like BioMart). 
Despite the advantage of most up-to-date information there may be some downsides :
Interogating databases may require more time to run all queries via interet-connections and this strategy is vulnerable to broken links (eg linked to the delay of updates between different types of databases that may need to get combined). Furthermore, the results typically change a bit when the queries get repeated. In this case one should rather search version-numbers for the specific queries.
...
Note, that tracking version-numbers may be easier.
...
The author would like to acknowledge the support by the [IGBMC](https://www.igbmc.fr/) (CNRS UMR 7104, Inserm U 1258, UdS), [CNRS](http://www.cnrs.fr/), [Universit&eacute; de Strasbourg](https://www.unistra.fr) and [Inserm](https://www.inserm.fr/).
