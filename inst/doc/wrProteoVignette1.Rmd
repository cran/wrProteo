...
The aim of package-vignettes is to provide additional information how the R-package concerned may be used, thus complementing the documentation given with _help()_ for each of the functions of the package. In terms of examples, frequent standard types of problems are preferred in a vignette. 
...
```{r setup32, echo=TRUE, eval=FALSE}
...
Below in this vignette, we'll use only the quantitation data generated using MaxQuant (ProteomeDiscoverer, Proline and OpenMS are supported, too). 
...
As further information about the proteins identifed can be considered during normalization it is, eg, possible to exclude contaminants.
In proteomics experiments it is very common to find contaminants like keratins among the higher abundant proteins which may introduce bias at normalization.
...
Biological replicates, however, make additional information accessible, allowing an interpretation of experiments in a more general way.
...
Data exported from [Proteome Discoverer](https://www.thermofisher.com/order/catalog/product/OPTON-30812) can get imported (typically the xx_allProteins.txt file) using `readProtDiscovFile()`, for details please see the vignette "UPS-1 spike-in Experiments" to this package. 


#### Read Data From Proline

[Proline](http://www.profiproteomics.fr/proline/) is free software provided by the Profi-consortium, 
see [Bouyssie et al 2020](https://doi.org/10.1016/j.jprot.2015.11.011).
Data exported from [Proline](http://www.profiproteomics.fr/proline/) (xlsx or tsv format) can get imported using `readProlineFile()`, for details please see the  vignette "UPS-1 spike-in Experiments" to this package. 


#### Read Data From OpenMS

[OpenMS](https://www.openms.de/) is free software provided by the deNBI Center for integrative Bioinformatics, 
...
see [Ritchie et al 2015](https://doi.org/10.1093/nar/gkv007). 
...
```{r VolcanoPlot1MQ, fig.height=7, fig.width=9.5, fig.align="center", echo=TRUE}
...
Tables with results can be either directed created using _VolcanoPlotW()_ or, as shown below, using `extractTestingResults()`.

For this example the moderated t-test expressed as Benjamini-Hochberg FDR gave `r sum(testMQ$BH[,1] < 0.05,na.rm=TRUE)` proteins with
FDR < 0.05 (however, many of them with low FC) for the comparison `r colnames(testMQ$BH)[1]`.
So one should add an additional filter for too low FC values.

```{r results1, echo=TRUE}
res1 <- extractTestingResults(testMQ, compNo=1, thrsh=0.05, FCthrs=2)
```
After FC-filtering for 2-fold (ie change to double or half) `r nrow(res1)` proteins remain.
...
Such a list of significant proteins from a given project could be tested lateron for enrichment of [GO-functions](http://geneontology.org/) or for their inter-connectivity in PPI networks like [String](https://string-db.org/).
...
The author would like to acknowledge the support by the [IGBMC](http://www.igbmc.fr/) (CNRS UMR 7104, Inserm U 1258), [CNRS](http://www.cnrs.fr/), [Universite de Strasbourg](https://www.unistra.fr) and [Inserm](https://www.inserm.fr/).
