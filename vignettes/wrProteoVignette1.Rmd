...
## Meta-Data (SDRF Format)  {#sdrf-Meta-Data}

Before importing quantification results, let's try to gather information about the experimental setup :

The project [Proteomics Sample Metadata Format](https://github.com/bigbio/proteomics-metadata-standard) aims to provide a framework 
...
Either you download the meta-data as file 'sdrf.tsv' from [Pride/PXD001819](https://www.ebi.ac.uk/pride/archive/projects/PXD001819), or you may read it directly from [github/bigbio](https://github.com/bigbio/proteomics-metadata-standard/blob/master/annotated-projects/PXD001819/PXD001819.sdrf.tsv).
If your dataset has been added to github, you don't necessarily need to copy/paste the full path, you only need the main [PRIDE](https://www.ebi.ac.uk/pride/) accession number, `readSdrfAnnot` can compose the full url for you.
...
str(pxd001819meta)
```
The **second vignette** (entitled "**UPS1 spike-in Experiments**") to [this package](https://CRAN.R-project.org/package=wrProteo) shows more details how this information can be used to in further analysis.
...
### MaxQuant: Protein Quantification Data    {#ReadMaxQuant}
...
Similarly we can also add directly here information about principal species, c
ontaminants and add sdrf annotation (if existing) directly when reading the data.
(This time let's do not display the plot of distributions, they are the same as above.)
...
To treat the data from an experiment one also need to declare who is replicate of whom.
The import-functions try to help you in multiple ways to find out more about the experimental details.
Annotation data exported by quantitation software lime MaxQuant or ProteomeDiscoverer can be read and minded for understanding theexperimental setup.
If sfdr-annotation exists on Pride, it can be imported too. Or the user may simply provide a data.frame formatted like sfdr from Pride.
If nothing of the above is available, the column-names from the quantitation columns will be minded to search hints about groups of replicates (in particular when using MaqQuant).
Finally, all functions have a _gr_ argument allowing to specify a specific setup.

In this simple example case, no additional annotation files, neither Sdrf were provided, we can use the information from column-names from the quantitation columns.
For a more complex example of using _readMaxQuantFile()_ or integrating other annotation information,
...
#### MaxQuant : Peptide Data   {#ReadMaxQuantPetides}
...
### ProteomeDiscoverer : Protein Quantification   {#ReadProteomeDiscoverer}
...
using `readProtDiscovFile()`, for details please see the vignette "UPS-1 spike-in Experiments" also available with [this package](https://CRAN.R-project.org/package=wrProteo).
...
For a more complex example of using _readProtDiscovFile()_ please see the vignette _'UPS1 spike-in Experiments'_ of [this package](https://CRAN.R-project.org/package=wrProteo).


#### ProteomeDiscoverer : Peptide Data       {#ReadProteomeDiscovererPetides}
...
### Proline : Protein Quantification Data         {#ReadProline}
...
Data exported from [Proline](https://www.profiproteomics.fr/proline/) (xlsx or tsv format) can get imported using `readProlineFile()`.
...
### MassChroQ : Protein Quantification Data
...
### Fragpipe : Protein Quantification Data       {#ReadFragpipe}

[Fragpipe](https://fragpipe.nesvilab.org/) is a database search tool for peptide identification, free software devolped by the [Nesvizhskii lab](https://www.nesvilab.org/),
see eg [da Veiga Leprevost; et al 2020](https://doi.org/10.1038/s41592-020-0912-y) and other related publications.
...
### OpenMS : Protein Quantification Data
...
### Adding Meta-Data To Main Data

With most import-functions the experimental setup can be directly imported as [sdrf-format](#sdrf-Meta-Data) (if available),
or from files produced using the various quantitation software (eg [MaxQuant](#ReadMaxQuant).
Most standard import functions, like _readMaxQuantFile()_, have these options already intergrated.
If you need to read other annotation files, you may use `readSampleMetaData()`.
In this case the user should make sure, the annotation really corresponds to the experimental data !
...
```{r readSampleMetaData1,  echo=TRUE}
specPrefMQ <- list(conta="CON_|LYSC_CHICK", mainSpecies="OS=Saccharomyces cerevisiae")
dataMQ <- readMaxQuantFile(path1, specPref=specPrefMQ, sdrf="PXD001819", suplAnnotFile=TRUE, plotGraph=FALSE)
```

As separate steps, the sdrf-meta-data can be first gathered like shown below.
When adding the quantitation-data in argument _abund_, the functions also check if the number of samples fit and
try to align the order of the meta-data to the qunatitation data (based on the raw files) since they are not necessarily in the same order.
...
Interogating databases may require more time to run all queries via interet-connections and this strategy is vulnerable to broken links (eg linked to the delay of updates between different types of databases that may need to get combined).
...
Annotation-tables from [UCSC](https://genome.ucsc.edu/cgi-bin/hgTables) are available for a decent number of species and can be downloaded for conventient off-line search. Howwever, in the context of less common species we realized that the UniProt tables from UCSC had many times low yield in final matching.
...
The author would like to acknowledge the support by the [IGBMC](https://www.igbmc.fr/) (CNRS UMR 7104, Inserm U 1258, UdS), [CNRS](http://www.cnrs.fr/), [Universit&eacute; de Strasbourg (UdS)](https://www.unistra.fr) and [Inserm](https://www.inserm.fr/).
