...
This package contains a collection of various tools for Proteomics used at the [proteomics platform](https://www.igbmc.fr/en/plateformes-technologiques/translate-to-english-proteomique) of the [IGBMC](https://www.igbmc.fr).
...
As described with [MaxQuant](#ReadMaxQuant), additional meta-data as [sdrf](#ImportSdrf) can be imported in the same way.
...
see also [Bouyssi&eacute; et al 2020](https://doi.org/10.1016/j.jprot.2015.11.011).
...
### Ionbot : Import Peptide Identification And  Quantification Data       {#ReadIonbot}

[ionbot](https://ionbot.cloud/about) is a database search tool for protein and peptide identification, developed by the [ComPomics group](https://www.compomics.com/) headed by L. Martens,
see eg  [Kong et al 2017](https://doi.org/10.1101/2021.07.02.450686) or other related publications.
Peptide identification from [ionbot](https://ionbot.cloud/about) are typically complemeented by quantification using [FlashLFQ](https://doi.org/10.1021/acs.jproteome.7b00608). 
The resulting output in tsv format can get imported using `readIonbotPeptides()`.  
Since the output may not have complete protein annotation information, it is possible to complement annotations from the fasta-file (Uniprot-formatted) used for initial identifications (using the argument '_specPref_', not shown below).
The example below is just a tiny toy data-set, normally one can identify and quantify many more proteins.


```{r readIonbot1,  echo=TRUE}
path1 <- system.file("extdata", package="wrProteo")
...
The user should pay attention to which identifyer should be used and that these identifyer do not appear multiple times in the the same data-set. 
...
This may may have noticable effect on the following biological interpretation.

Thus, it is very important to know your data and to understand that lines appearing with the same identifyers should/may be fused/summarized without 
...
All collegues from the [proteomics platform](https://www.igbmc.fr/en/plateformes-technologiques/translate-to-english-proteomique) at the IGBMC work very commited to provide high quality mass-spectrometry data (including some of those used here).
...
For completeness : The output from \code{sessionInfo()}
