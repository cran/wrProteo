...
[MassChroQ](http://pappso.inrae.fr/bioinfo/masschroq/) and [OpenMS](https://www.openms.de).
...
In summary, the content of argument _specPref_ will get searched in multiple types of annotation (if available from the initial Fasta).
...
If sfdr-annotation (see [Proteomics Sample Metadata Format](https://github.com/bigbio/proteomics-metadata-standard)) exists on [Pride](https://www.ebi.ac.uk/pride/), it can be imported, too.
...
However, the recommended and most convenient way is to add/import meta-data directly when importing quantitation-data (eg using _readMaxQuantFile()_, _readProtDiscovFile()_, etc).
