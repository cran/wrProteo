...
Here we will read a tiny example fasta-file obtained from a collection of typical contaminants in proteomics using `readFasta2()`.
...
```{r writeFasta, echo=TRUE, eval=FALSE}
...
As stated in other places, PCA can't handle missing values (ie _NA_s). 
...
Furthermore, the results typically may change a bit when the queries get repeated. 
...
Besides, the tables [UCSC](https://genome.ucsc.edu/cgi-bin/hgTables) are another option.
Note, that tracking version-numbers may be much easier using this approach.
