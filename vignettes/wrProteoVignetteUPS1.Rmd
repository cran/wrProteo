...
Please note, that in the example above we directly added information about the experimental setup from the sdrf repository.
Given the annotation the non-parsed anotation from the fasta is available, we wrote the _specPref_ argument slightly different than in the other examples.
...
The resultant data were export in tablulated format and the file automatically named '\_Proteins.txt_' (the option R-headers was checked, however this option is not mandatory).
Files compressed as .gz can be read, too (like in the example below).
...
In the example below both the default annotation as file _'InputFiles.txt'_ and sdrf annotation are available and were integrated to object produced by the import-function.

If the user chooses to add other custom sample-names, much care should be taken on the order of the the vector _sampleNames_ .
...
(see also [proline-core on github](https://github.com/profiproteomics/proline-core)), published by [BouyssiÃ© et al 2020](https://doi.org/10.1093/bioinformatics/btaa118).
...
In addition, we need to correct the quantification column-heads (like â€˜Levure2ug+ UPS1-100amol-1â€™) and bring them to a simpler version :
...
On this PCA one can see the three software types used in red.
...
disperse/distinct group of low concentration-pairs (separated by PC1, mostly containing at least one 50 or 500fmol), low/med conc pairs (containing 2500, ev 5000)
and the rest (med+high to any, high degree of text-superposition).
Furthermore,
...
[CNRS](http://www.cnrs.fr), [Universit&eacute; de Strasbourg](https://www.unistra.fr) and [Inserm](https://www.inserm.fr)
